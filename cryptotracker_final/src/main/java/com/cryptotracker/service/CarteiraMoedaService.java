...
public class CarteiraMoedaService {
    ...
    public CarteiraMoedaService(CarteiraMoedaRepository repo, CarteiraRepository carteiraRepo, MoedaRepository moedaRepo){
        this.repo=repo; this.carteiraRepo=carteiraRepo; this.moedaRepo=moedaRepo;
    }
    public CarteiraMoeda adicionar(Long carteiraId, Long moedaId, double quantidade){
        carteiraRepo.findById(carteiraId).orElseThrow(()->new ResourceNotFoundException("ERR_CARTEIRA_NOT_FOUND","Carteira não encontrada"));
        moedaRepo.findById(moedaId).orElseThrow(()->new ResourceNotFoundException("ERR_MOEDA_NOT_FOUND","Moeda não encontrada"));
        var exists = repo.findByCarteiraIdAndMoedaId(carteiraId, moedaId);
        if(!exists.isEmpty()){ var c = exists.get(0); c.setQuantidade(c.getQuantidade()+quantidade); return repo.save(c); }
        CarteiraMoeda cm = new CarteiraMoeda(carteiraId, moedaId, quantidade);
        return repo.save(cm);
    }
    public List<CarteiraMoeda> listarPorCarteira(Long carteiraId){ return repo.findByCarteiraId(carteiraId); }
    ...
    public CarteiraMoeda alternarFavorito(Long carteiraId, Long moedaId){
        ...
        if(list.isEmpty()) throw new ResourceNotFoundException("ERR_CM_NOT_FOUND","Registro Carteira-Moeda não encontrado");
        var cm = list.get(0);
